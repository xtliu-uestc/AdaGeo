...
def get_XY(dataset):
    ...
    if dataset == "Shanghai":
        ...
        X_isp = data['isp']
        X_isp = preprocessing.LabelEncoder().fit_transform(X_isp)
        X_isp = preprocessing.MinMaxScaler().fit_transform(np.array(X_isp).reshape((-1, 1)))

       
        X_ip = data[['ip_split1', 'ip_split2', 'ip_split3', 'ip_split4']]
        X_ip = preprocessing.MinMaxScaler().fit_transform(np.array(X_ip))

       
        X_as = data['asnumber']
        X_as = preprocessing.LabelEncoder().fit_transform(X_as)
        X_as = preprocessing.MinMaxScaler().fit_transform(np.array(X_as).reshape(-1, 1))

        X_ping = data[['aiwen_ping_delay_time', 'vp806_ping_delay_time', 
                       'vp808_ping_delay_time', 'vp813_ping_delay_time']]
        delay_scaler = MaxMinScaler()
        delay_scaler.fit(X_ping)
        X_ping = delay_scaler.transform(X_ping)

        X_hop = data[['aiwen_tr_steps', 'vp806_tr_steps', 'vp808_tr_steps', 'vp813_tr_steps']]
        step_scaler = MaxMinScaler()
        step_scaler.fit(X_hop)
        X_hop = step_scaler.transform(X_hop)
        ...
        X_last[X_last <= 0] = 0
        X_last = preprocessing.MinMaxScaler().fit_transform(X_last)

       
        X_onehot = data[['orgname', 'asname', 'address', 'isp']]
        scaler = preprocessing.OneHotEncoder(sparse_output=False)
        X_onehot = scaler.fit_transform(X_onehot)

       
        X_shared = np.concatenate([X_ip, X_isp, X_as, X_ping, X_hop, X_last], axis=1)  # 30维
        X_extra = X_onehot  

        X = np.concatenate([X_shared, X_extra], axis=1)
        
        print(f"上海特征: 共享{X_shared.shape[1]}维 + 特有{X_extra.shape[1]}维 = {X.shape[1]}维")
...
def get_graph(dataset, lm_idx, tg_idx, mode):
    X, Y, T = get_XY(dataset) 

    last_hop = list(map(find_nearest_router, T)) 
