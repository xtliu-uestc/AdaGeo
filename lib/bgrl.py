...
class BGRL(torch.nn.Module):
    ...
    def forward(self, online_x, target_x, drop_edges=True):
        # forward online network
        online_y_pred,online_y = self.online_encoder(online_x)
        ...
        with torch.no_grad():
            target_y_pred,target_y = self.target_encoder(target_x)
            target_y=target_y.detach()
        return online_q, target_y, online_y_pred
